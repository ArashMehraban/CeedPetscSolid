...
CEED_QFUNCTION(SetupMMSForce)(void *ctx, const CeedInt Q,
  ...
  for (CeedInt i=0; i<Q; i++) {
    ...
    // force[i+0*Q] = -wJ*((E*nu*y)/((2.*nu - 1.)*(nu + 1.)) + (E*nu*z)/((2.*nu - 1.)*(nu + 1.)) - (E*y*(nu - 1./2.))/(2.*(2.*nu - 1.)*(nu + 1.)) - (E*z*(nu - 1./2.))/(2.*(2.*nu - 1.)*(nu + 1.)));
    // force[i+1*Q] = -wJ*((E*nu*x)/((2.*nu - 1.)*(nu + 1.)) + (E*nu*z)/((2.*nu - 1.)*(nu + 1.)) - (E*x*(nu - 1./2.))/(2.*(2.*nu - 1.)*(nu + 1.)) - (E*z*(nu - 1./2.))/(2.*(2.*nu - 1.)*(nu + 1.)));
    // force[i+2*Q] = -wJ*((E*nu*x)/((2.*nu - 1.)*(nu + 1.)) + (E*nu*y)/((2.*nu - 1.)*(nu + 1.)) - (E*x*(nu - 1./2.))/(2.*(2.*nu - 1.)*(nu + 1.)) - (E*y*(nu - 1./2.))/(2.*(2.*nu - 1.)*(nu + 1.)));

//PetscPrintf(PETSC_COMM_WORLD,"wJ %f\n", wJ);
force[i+0*Q] = wJ*((E*(nu-1.0)*-2.0)/((nu*2.0-1.0)*(nu+1.0))-(E*(nu-1.0/2.0)*2.0)/((nu*2.0-1.0)*(nu+1.0)));
//PetscPrintf(PETSC_COMM_WORLD,"force[i+0*Q] %f\n", force[i+0*Q]);
force[i+1*Q] = wJ*((E*(nu-1.0)*-2.0)/((nu*2.0-1.0)*(nu+1.0))-(E*(nu-1.0/2.0)*2.0)/((nu*2.0-1.0)*(nu+1.0)));
force[i+2*Q] = wJ*((E*(nu-1.0)*-2.0)/((nu*2.0-1.0)*(nu+1.0))-(E*(nu-1.0/2.0)*2.0)/((nu*2.0-1.0)*(nu+1.0)));
...
/*
    ...
    force[i+0*Q] = wJ*(-(E*(cos(x*2.0)*cos(y*3.0)*exp(z*4.0)*4.0-cos(z*4.0)*sin(y*3.0)*exp(x*2.0)*8.0)*(nu-0.5))/((nu*2.0-1.0)*(nu+1.0))+
                        ...
                        (E*cos(z*4.0)*sin(y*3.0)*exp(x*2.0)*(nu-1.0)*4.0)/((nu*2.0-1.0)*(nu+1.0)))/1e8;
    ...
    force[i+1*Q] = wJ*(-(E*(cos(y*3.0)*cos(z*4.0)*exp(x*2.0)*3.0-cos(x*2.0)*sin(z*4.0)*exp(y*3.0)*2.0)*(nu-0.5))/((nu*2.0-1.0)*(nu+1.0))+
                        ...
                        (E*cos(x*2.0)*sin(z*4.0)*exp(y*3.0)*(nu-1.0)*9.0)/((nu*2.0-1.0)*(nu+1.0)))/1e8;
    ...
    force[i+2*Q] = wJ*(-(E*(cos(x*2.0)*cos(z*4.0)*exp(y*3.0)*6.0-cos(y*3.0)*sin(x*2.0)*exp(z*4.0)*(4.5))*(nu-0.5))/((nu*2.0-1.0)*(nu+1.0))+
                        ...
                        (E*cos(y*3.0)*sin(x*2.0)*exp(z*4.0)*(nu-1.0)*16.0)/((nu*2.0-1.0)*(nu+1.0)))/1e8;
*/
