...
// -----------------------------------------------------------------------------
CEED_QFUNCTION(HyperSSF)(void *ctx, CeedInt Q, const CeedScalar *const *in, CeedScalar *const *out) {
   // *INDENT-OFF*
   // Inputs
...
// -----------------------------------------------------------------------------
CEED_QFUNCTION(HyperSSdF)(void *ctx, CeedInt Q, const CeedScalar *const *in, CeedScalar *const *out) {
   // *INDENT-OFF*
   // Inputs
    ...
    //Above Voigt Notation is placed in a 3x3 matrix:
     ...
     for (int j=0; j<3; j++)
       for (int k=0; k<3; k++) {
         deltavg[j][k][i] = 0;
         for (int m=0; m<3; m++)
           deltavg[j][k][i] += dXdx[m][j] * dsigma[k][m] * wJ;
       }
     } // End of Quadrature Point Loop
